...
namespace Rainbow.ImgLib.Encoding.Implementation
{
    public class ColorCodec24BitRGB : ColorCodec
    {
        public override SKColor[] DecodeColors(byte[] palette, int start, int size)
        {
            ...
            {
                pal.Add(new SKColor(palette[start + i * 3], palette[start + i * 3 + 1], palette[start + i * 3 + 2]));
            }
        ...
        public override byte[] EncodeColors(SKColor[] colors, int start, int length)
        {
            ...
            {
                palette[(i - start) * 3] = colors[i].Red;
                palette[(i - start) * 3 + 1] = colors[i].Green;
                palette[(i - start) * 3 + 2] = colors[i].Blue;
            }
            return palette;
        }
